...
4. **Protocol** (the whole skill):
   ...
   - **Intake, conversational** — learn task, familiarity, depth, time in one or two natural messages; if waved off, state inferred defaults in one line and go.
   ...
   - **Stops** — one concept per message, ≤300 words / one screen, essence→particulars, anchor to doc location (§/p.), progress marker ("Stop 2 of 7"), ask-or-proceed gate.
...
`briefing` = the man on the call: full read → conversational intake → message 1 with big picture + task-driven stop map + declared skips → one-concept ≤300-word anchored stops with ask-or-proceed gates → questions answered immediately, deep dives as chunked detours → announced re-planning → TLDR recap mapping back into the doc. Genre-blind by design: pure interaction protocol, zero domain examples.
